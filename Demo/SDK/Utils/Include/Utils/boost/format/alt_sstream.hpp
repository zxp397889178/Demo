...
namespace tiny_boost {
    namespace io {
		...
		//zx£º´úÌæbase_from_member£¬
		template <class Ch, class Tr, class Alloc>
		class pbase_oaltstringstream
		{
			typedef basic_altstringbuf<Ch, Tr, Alloc>	stringbuf_t;
		public:
			::std::shared_ptr<stringbuf_t> buf_ptr;

			template <typename ...T>
			explicit pbase_oaltstringstream(T&& ...x)
				: buf_ptr(static_cast<T&&>(x)...)
			{}
		};
...
// ---   class basic_oaltstringstream ----------------------------------------
        ...
        class basic_oaltstringstream 
			...
			private pbase_oaltstringstream<Ch, Tr, Alloc>,
              public ::std::basic_ostream<Ch, Tr>
        {
            ...
            //typedef tiny_boost::base_from_member<tiny_boost::shared_ptr<
            //    basic_altstringbuf<Ch,Tr, Alloc> > > 
            //    pbase_type;
			typedef pbase_oaltstringstream<Ch, Tr, Alloc> pbase_type;
            typedef ::std::basic_string<Ch, Tr, Alloc>  string_type;
            typedef typename string_type::size_type     size_type;
            typedef basic_altstringbuf<Ch, Tr, Alloc>   stringbuf_t;
        public:
            ...
            basic_oaltstringstream() 
                : pbase_type(new stringbuf_t), stream_t(buf_ptr.get())
                { }
            basic_oaltstringstream(::std::shared_ptr<stringbuf_t> buf) 
                : pbase_type(buf), stream_t(buf_ptr.get())
                { }
            basic_oaltstringstream(stringbuf_t * buf) 
				: pbase_type(buf, No_Op()), stream_t(buf_ptr.get())
                { }
