...
# else
                                      ...
                                      std::size_t pos, std::size_t size)
    {
		if (exceptions & io::bad_format_string_bit)
			tiny_boost::throw_exception(io::bad_format_string(pos, size) );
    }
                                ...
                                std::size_t offset, unsigned char exceptions)
    {
      ...
      // argument type modifiers
        while (start != last) {
            switch (wrap_narrow(fac, *start, 0)) {
                ...
                case 'z':
                case 'L':
                    // tiny_boost::format ignores argument type modifiers as it relies on
                    // the type of the argument fed into it by operator %
                    break;

                // Note that the ptrdiff_t argument type 't' from C++11 is not honored
                // because it was already in use as the tabulation specifier in tiny_boost::format
                // case 't':
      ...
      parse_conversion_specification:
        ...
        switch (wrap_narrow(fac, *start, 0))
        {
            ...
            // Tabulation (a tiny_boost::format extension)
            case 'T':
                ++start;
                if( start >= last) {
                    maybe_throw_exception(exceptions, start-start0+offset, fstring_size);
            ...
            // %n is insecure and ignored by tiny_boost::format
            case 'n' :
                                 ...
                                 unsigned char exceptions)
    {
        ...
        using namespace tiny_boost::io;
        typename String::size_type i1=0;
        int num_items=0;
        while( (i1=buf.find(arg_mark,i1)) != String::npos ) {
            if( i1+1 >= buf.size() ) {
				if (exceptions & bad_format_string_bit)
					tiny_boost::throw_exception(bad_format_string(i1, buf.size() )); // must not end in ".. %"
                else {
                  ++num_items;
                  break;
                }
