...
#elif XD3_WIN32
  int wcsLen = MultiByteToWideChar(CP_UTF8, NULL, name, strlen(name), NULL, 0);
  wchar_t* wszString = new wchar_t[wcsLen + 1];

  MultiByteToWideChar(CP_UTF8, NULL, name, strlen(name), wszString, wcsLen);
  wszString[wcsLen] = '\0';

  wstring strUnicode = wszString;
  delete wszString;

  xfile->file = CreateFileW(strUnicode.c_str(),
